...
# Description: Input variable declarations for AI Agent Service BYO storage lab
...
variable "search_sku" {
  description = "SKU tier for the AI Search service"
  type        = string
  default     = "basic"

  validation {
    condition     = contains(["free", "basic"], var.search_sku)
    error_message = "Search SKU must be free or basic."
  }
}
