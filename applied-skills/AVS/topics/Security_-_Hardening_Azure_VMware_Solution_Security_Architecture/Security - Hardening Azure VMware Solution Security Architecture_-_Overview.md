...
| --------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Physical hardware     | Customer owns and operates the hosts.                                             | Microsoft owns and operates the hosts.                                                      |
| Host lifecycle        | Customer replaces failed hosts and manages hardware maintenance.                  | Microsoft detects failures, evacuates workloads, and replaces bare-metal hosts.             |
| Root ESXi access      | A customer administrator may have direct root-level access.                       | The tenant is intentionally denied unrestricted root-level access.                          |
| vCenter authority     | The administrator commonly controls the entire vSphere domain.                    | The customer receives a restricted `cloudadmin@vsphere.local` account.                      |
...
| NSX Tier-0 gateway    | The customer may own the complete routing stack.                                  | Microsoft controls the Tier-0 gateway that connects AVS to the Azure backbone.              |
...
* **Intended use:** The account is described as a break-glass emergency identity rather than as the standard identity for daily operations.
...
The transcript states that the AVS cloud administrator cannot add an external identity source directly through the usual vCenter interface. Instead, directory integration is performed through Azure run commands.

* **Control-plane path:** The requested operation is submitted through Azure Resource Manager and executed through the Azure control plane.
...
* **Trigger condition:** A corporate policy requires the local account password to be changed, such as every 90 days.
...
> **Operational recommendation:** Create an individual service account for each third-party integration. A machine identity should never depend on the emergency human-administration account.
...
| ------------------------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `abs-fleet-rp`                  | Configuration injection, hardware lifecycle management, and vCenter health monitoring. | `AVS orchestrator` at the Azure resource-group level. |
| `azs-vis-prod-app`              | AVS fleet or visibility-related management operations.                                 | `AVS on fleet-vis` at the Azure resource-group level. |

* **Dependency:** Both applications must remain enabled.
* **RBAC scope:** Their role-based access control assignments are described as being applied at the Azure resource-group level.
...
> **Requires documentation validation:** The exact application names and RBAC role labels may have been affected by transcription. Validate `abs-fleet-rp`, `azs-vis-prod-app`, `AVS orchestrator`, and `AVS on fleet-vis` before using them in an implementation.
...
> **Requires documentation validation:** The command name `setAzureAidServicePrinciple` appears likely to be a transcription error. Do not execute a command based on this spelling without verifying the supported AVS recovery procedure.
...
* **Tenant capability:** Customers can create workload segments, configure Tier-1 functions, and apply microsegmentation policies.
...
* **Role-integration behavior:** External directory groups can be mapped to supported NSX roles such as auditor, VPN administrator, or network operator.
...
An HCX network extension stretches a local Layer 2 virtual LAN from an on-premises network into AVS through a secured tunnel.

* **Migration benefit:** A virtual machine can move from an on-premises environment into AVS without changing its IP address.
...
* **Default posture:** The transcript states that public network access is disabled by default.
...
* **Private transport:** Azure ExpressRoute provides private connectivity between customer infrastructure and Azure.
* **Global Reach function:** ExpressRoute Global Reach is described as allowing on-premises routers and AVS connectivity to communicate directly.
...
The baseline architecture in the transcript specifies Azure Firewall Premium rather than Standard because Premium provides advanced inspection capabilities.
...
* **IDPS mode:** The transcript specifies configuring IDPS in deny mode so malicious traffic is blocked rather than merely logged.
...
* Client systems must trust the certificate authority used by the firewall.
* Certificate deployment and rotation must be governed.
* Applications using certificate pinning may reject intercepted sessions.
...
* **Distributed firewall position:** NSX applies policy at or near the virtual network interface of each virtual machine.
...
A secure private cloud eventually needs controlled entry points for public applications. Azure Application Gateway provides Layer 7 routing and Web Application Firewall protection in front of workloads hosted on AVS virtual machines.
...
* **Layer 7 behavior:** Application Gateway reads HTTP information and makes content-aware routing decisions.
...
* **Inspection scope:** The WAF evaluates HTTP headers and request bodies.
* **Threat examples:**
...
9. Use health probes to ensure that requests reach only healthy backends.
...
The transcript describes deploying either the Azure Monitor Agent or the legacy Log Analytics Agent inside AVS guest operating systems.
...
* **Transport:** The agent streams telemetry from the AVS workload to an Azure Log Analytics workspace.
...
> **Requires documentation validation:** The current support and retirement status of the legacy Log Analytics Agent must be checked before deployment. The transcript presents it as an available option but does not establish whether it remains recommended.

## 8.2 Microsoft Defender for Cloud

Defender for Cloud provides an analysis and posture-management layer over the centralized telemetry.
...
Microsoft Sentinel is described as the Security Information and Event Management platform that correlates alerts across the enterprise.
...
* **Response value:** Automated playbooks can act without waiting for manual intervention.
...
The transcript presents a Kusto Query Language (KQL) example that searches Windows Security Event ID `4625`, which represents a failed sign-in.
...
| Customer-managed key | The customer creates and governs the KEK in Azure Key Vault. | Higher             | Regulatory, contractual, or internal requirements for direct cryptographic control. |
...
* Standard RSA keys.
* RSA keys backed by a Hardware Security Module (HSM).
* Key sizes of 2048, 3072, or 4096 bits.
...
* **Identity creation:** Enable a system-assigned managed identity on the AVS private cloud.
...
* **Retention stated in transcript:** 90 days.
...
* **Detection timing stated in transcript:** AVS detects a new key version within approximately 10 minutes.
...
* **Vulnerability prioritization:** The Common Vulnerability Scoring System is used to assess security findings.
...
* **Standards named in the transcript:**
  ...
  * Various International Organization for Standardization certifications.
...
Encryption protects dormant data, but the operating system and boot process become exposed when a virtual machine starts. Trusted Launch addresses bootkits, tampered kernels, credential theft, and kernel-level compromise through three principal controls:
...
Secure Boot creates a verified startup chain using Unified Extensible Firmware Interface firmware and trusted digital signatures.
...
A virtual Trusted Platform Module, or vTPM, emulates TPM 2.0 capabilities for a virtual machine. It stores and uses cryptographic secrets within a boundary that is logically separated from the guest operating system.
...
The transcript states that AVS customers do not need to configure an external key-management server merely to add a vTPM.
...
* **Storage location:** Measurements are extended into Platform Configuration Registers within the vTPM.
...
* **Key-storage function:** BitLocker can protect volume key material using the vTPM.
...
Virtualization-Based Security, or VBS, uses hypervisor-enforced memory isolation to protect critical Windows security functions from the normal operating-system kernel.
...
Credential Guard uses VBS to isolate authentication secrets handled by the Local Security Authority.
...
Hypervisor-Protected Code Integrity, or HVCI, uses the VBS secure environment to protect code-integrity decisions.
