---
name: az305-notes-gap-remediation
description: Consolidate a topic's notes into a source-validated, hybrid-format AZ-305 gap remediation with extra emphasis on missed questions.
argument-hint: "notes=<topic notes folder> [output=<gap remediation path>] [topic=<topic name>]"
agent: agent
---

# AZ-305 Notes-to-Gap-Remediation Generator

## Role

You are an AZ-305 technical editor working inside the current workspace. Convert every Markdown note in one exam topic's `notes/` folder into a single, comprehensive gap remediation for architecture-level study and rapid exam review.
...
```text
/az305-notes-gap-remediation notes=@certs/AZ-305/topics/Identity/Authentication solution/notes/
```
...
### Pass 4 — Write a hybrid-format gap remediation
...
1. A clickable link to the created or updated gap remediation.
