...
For this exercise, you'll take the **[Contoso Chat Retail copilot with Foundry & PromptFlow (Python)](https://aka.ms/contoso-retail-sample)** project template as your starting point. This project template is a code-first experience that uses Prompty and PromptFlow to build a custom copilot (chat AI) that can be integrated into the retail  website (chat UI) of a fictional company called Contoso Outdoors.
...
- Visiting the [Foundry portal](https://ai.azure.com) portal. Sign in using your Azure account. Search for the AI hub corresponding to the resource group above (you may need to refresh a few times). Select the listed AI project, then select **Deployments** in its sidebar to track status for models and chat application deployments.
...
1. Let's start by verifying that the key [Foundry architecture](https://learn.microsoft.com/azure/ai-studio/concepts/architecture) resources were created. The figure below provides more details on what each of these resources provides to our AI application.
    ...
    ![Foundry architecture](./media/resource-provider-connected-resources.svg)
...
## Validate deployment using the Foundry

The Azure Portal helps you manage the underlying Azure resources for your project. The Foundry portal helps you *build and manage* the AI projects themselves, end-to-end, from model selection to application deployment. The `azd up` command should have completed the entire process from provisioning required models, to deploying and hosting the copilot API endpoint for usage. Let's validate that the application is functioning as expected.

1. Visit the **Manage** page in the [Foundry portal](https://ai.azure.com/manage) to view all Azure AI hubs in your subscription.
...
## Test the deployment (in the cloud) using Foundry

To validate that the deployed copilot works, use the built-in test playground capability in the Foundry portal.

![Chat deployment details](./media/chat-deployment-details.png)

1. In the Foundry portal, from the **App deployments** list, select the **chat-deployment-xxxx** deployment.
